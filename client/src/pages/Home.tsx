...
export default function Home() {
  ...
  return <div>
    ...
    <section className="section"><div className="container"><div className="section-head"><h2>각자의 현업 언어로<br />시스템을 만드는 강사진.</h2></div><div className="grid3">{instructors.map(i=><Link href={`/instructors/${i.slug}`} className="card instructor-card" key={i.slug}><div className="avatar">{i.name.slice(-1)}</div><span className="card-tag">{i.role}</span><h3>{i.name}</h3><p>{i.intro}</p><div className="card-meta"><span>PROFILE</span><ArrowUpRight size={15} /></div></Link>)}</div></div></section>
