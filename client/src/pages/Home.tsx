...
export default function Home() {
  ...
  return <div>
    ...
    <section className="section"><div className="container"><div className="section-head"><div className="eyebrow">01 / curriculum tracks</div><h2>도구를 배우는 대신,<br />업무의 구조를 다시 설계합니다.</h2><p>세 개의 실무 트랙은 사용하는 도구가 아니라, 반복되는 업무와 결과물을 중심으로 구성됩니다.</p></div>{tracks.map(t=><div className="track-row" key={t.code}><div className="track-code">{t.code}</div><div><h3>{t.title}</h3><p>{t.body}</p></div><div className="chips">{t.chips.map(c=><span className="chip" key={c}>{c}</span>)}</div></div>)}</div></section>
    <section id="process" className="section process"><div className="container"><div className="section-head"><div className="eyebrow">02 / how it works</div><h2>진단에서 운영까지,<br />한 번의 완성으로 끝나지 않게.</h2><p>학습이 끝난 뒤에도 다시 사용할 수 있도록 업무 맥락과 운영 기준까지 남깁니다.</p></div><div className="process-grid">{[["01","진단","반복 업무와 현재 흐름을 찾습니다."],["02","설계","결과물과 판단 기준을 구조화합니다."],["03","실습","실제 데이터로 시스템을 연결합니다."],["04","피드백","개인·팀 상황에 맞춰 다듬습니다."],["05","적용","현업에서 다시 움직이도록 운영합니다."]].map(([n,t,d])=><div className="process-step" key={n}><div className="mono">{n}</div><h4>{t}</h4><p>{d}</p></div>)}</div></div></section>
